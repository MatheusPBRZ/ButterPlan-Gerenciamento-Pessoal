...
<body>
        ...
        <main class="main-content">
            <header style="display:flex; justify-content:space-between; align-items:center; flex-wrap: wrap; gap:10px;">
                ...
                <div style="display:flex; gap:10px;">
                    <button onclick="openModal('installmentModal')" class="btn-save" style="width:auto; margin:0; padding:15px 15px; background:#2c3e50;"><i class="fa-solid fa-credit-card"></i> Parcelamento</button>
                    <button onclick="openModal('fixedModal')" class="btn-save" style="width:auto; margin:0; padding:15px 15px; background:#444;"><i class="fa-solid fa-repeat"></i> Conta Fixa</button>
                    <button onclick="openModal('financeModal')" class="btn-save" style="width:auto; margin:0; padding:15px 15px;"><i class="fa-solid fa-plus"></i> Transação</button>
                </div>
            </header>
                                    ...
                                    <li style="flex-direction: column; align-items: flex-start; opacity: <?= $parc->is_finished ? '0.5' : '1' ?>">
                                        <div style="display:flex; justify-content:space-between; width:100%; align-items:center;">
                                            ...
                                            <div style="display:flex; gap:10px; align-items:center;">
                                                <span style="font-size: 0.9rem; font-weight:bold; color: var(--accent-color);"><?= $parc->paid_installments ?>/<?= $parc->total_installments ?> pagas</span>
                                                <?php if(!$parc->is_finished): ?>
                                                    <button onclick="openPayInstModal(<?= $parc->id ?>, '<?= addslashes($parc->title) ?>', <?= $parc->installment_amount ?>)" style="background:none; border:1px solid var(--accent-color); color:var(--accent-color); cursor:pointer; padding: 4px 8px; border-radius: 4px; font-size:0.8rem;"><i class="fa-solid fa-check"></i> Pagar</button>
                                                <?php endif; ?>
                                                ...
                                                <a href="index.php?page=financas&action=delete_installment&id=<?= $parc->id ?>" onclick="return confirm('Apagar parcelamento?')" style="color:#666;"><i class="fa-solid fa-trash"></i></a>
                                            </div>
                                        </div>
                                        <div class="progress-bar-bg"><div class="progress-bar-fill" style="width: <?= $perc ?>%;"></div></div>
                                        
                                        <div id="hist-<?= $parc->id ?>" style="display:none; width:100%; margin-top:10px; padding:10px; background:rgba(0,0,0,0.2); border-radius:5px;">
                            ...
                            <?php if(empty($fixas)): ?><p style="color:#777;">Nenhuma conta fixa.</p><?php else: ?>
                                <?php foreach($fixas as $conta): ?>
                                    <li style="display:flex; justify-content:space-between; align-items:center; opacity: <?= $conta->is_paid ? '0.5' : '1' ?>">
                                        <div><strong><?= htmlspecialchars($conta->title) ?></strong><small style="display:block; color:#777">Vence dia <?= $conta->day_of_month ?></small></div>
                                        <div style="display:flex; gap:10px; align-items:center;">
                                            ...
                                            <?php if(!$conta->is_paid): ?>
                                                <form method="POST" action="index.php?page=financas" style="display:inline;">
                                                    ...
                                                    <input type="hidden" name="description" value="Pgmto: <?= $conta->title ?>">
                                                    <input type="hidden" name="amount" value="<?= $conta->amount ?>">
                                                    <button type="submit" style="background:none; border:1px solid #555; color:white; cursor:pointer; padding:2px 5px;"><i class="fa-solid fa-check"></i></button>
                                                </form>
                                            <?php else: ?>
                                                <i class="fa-solid fa-circle-check" style="color:var(--success-color)"></i>
                                            <?php endif; ?>
                                            <a href="index.php?page=financas&action=delete_fixed&id=<?= $conta->id ?>" style="color:#666;"><i class="fa-solid fa-trash"></i></a>
                                        </div>
                                    </li>
                                <?php endforeach; ?>
                            <?php endif; ?>
                    ...
                    <table class="simple-table" style="width:100%;">
                        <?php foreach($allTransactions as $t): ?>
                            <tr style="border-bottom:1px solid #333;">
                                <td style="padding:10px; color:#aaa; font-size:0.8rem;"><?= date('d/m', strtotime($t->created_at)) ?></td>
                                <td>
                                ...
                                <td style="text-align:right;"><a href="index.php?page=financas&action=delete_transaction&id=<?= $t->id ?>" style="color:#555;"><i class="fa-solid fa-trash"></i></a></td>
                            </tr>
                        <?php endforeach; ?>
                    </table>
                </div>
    ...
    <i class="fa-solid fa-xmark"></i>
            ...
            <form method="POST">
                ...
                <div class="form-group"><input type="text" name="description" placeholder="Ex: Mercado, Salário..." required></div>
                <div class="form-group"><input type="number" step="0.01" name="amount" placeholder="Valor (R$)" required></div>
    ...
    <i class="fa-solid fa-xmark"></i>
            ...
            <form method="POST">
                ...
                <div class="form-group"><input type="text" name="title" placeholder="Ex: Aluguel, Internet..." required></div>
                <div class="form-group"><input type="number" step="0.01" name="amount" placeholder="Valor (R$)" required></div>
                <div class="form-group"><input type="number" name="day_of_month" placeholder="Dia do Vencimento (1-31)" required min="1" max="31"></div>
    ...
    <i class="fa-solid fa-xmark"></i>
            ...
            <form method="POST">
                ...
                <div class="form-group"><label>Item Comprado</label><input type="text" name="title" placeholder="Ex: Monitor, Celular..." required></div>
                <div class="form-group"><label>Valor Total (R$)</label><input type="number" step="0.01" name="total_amount" required></div>
                <div style="display:flex; gap:10px;">
                    <div class="form-group" style="flex:1;"><label>Vezes</label><input type="number" name="total_installments" required min="2"></div>
                    <div class="form-group" style="flex:1;"><label>Dia Venc.</label><input type="number" name="due_day" required min="1" max="31"></div>
                </div>
    ...
    <div id="payInstModal" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header"><h3>Pagar Parcela</h3><button class="close-modal" onclick="closeModal('payInstModal')">
    <i class="fa-solid fa-xmark"></i>
</button></div>
            <form method="POST">
